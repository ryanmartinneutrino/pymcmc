...
class MCMC:
    ...
    def run(self,nmcmc=1000,burnin=100):
        ...
        print "starting MCMC with",self.nmcmc," iterations and",self.burnin,"burn-in"
        ...
        previousL=self.get_likelihood(self.current_pars)
        for step in range(self.nmcmc):
            testPars=self.get_proposal_pars(self.current_pars)#trial parameters
            testL=self.get_likelihood(testPars)#trial likelihood
